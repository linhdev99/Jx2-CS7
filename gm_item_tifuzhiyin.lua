...
function Open()
    ...
    if nRoute <= 0 then
        ...
        tSay = {
            ...
            "NhËn trang bÞ /GetTB", "NhËn thó c­ìi /Mod_GetThuCuoi",
...
function Mod_GetThuCuoi()
    local szSay = {
        g_szTitle .. "Chän thó c­ìi:", "B¹ch M·/#Mod_GetThuCoi(34)",
...
function Boss_ThuongThan()
    ...
    local nMap, nX, nY = GetWorldPos();
    local npcIndex = 0
    npcIndex = CreateNpc("HuyÒn Vâ Th­ong", "Th­¬ng ThÇn Doanh Thiªn",
        nMap, nX + 2, nY + 2, -1, 1, 1, 30)
...
function Boss_ThitNuong()
    ...
    for i = 1, 20 do
        nAddX = random(-70, 70)
        nAddY = random(-70, 70)
        nTargetNpc = CreateNpc("M©m cç", "ThÞt N­íng", nMap, nX + nAddX,
            nY + nAddY)
...
LSB_NPC_GOLD_BOSS = {
    ...
    {
        "Hoa Hßa Th­îng Lç TrÝ Th©m", 167,
        "Hoa Hßa Th­îng Lç TrÝ Th©m", 30 * 60
        ...
        "NhÊt Tr­îng Thanh Hæ Tam N­¬ng", 167,
        "NhÊt Tr­îng Thanh Hæ Tam N­¬ng", 30 * 60
    }, {
        "Song Th­¬ng T­íng §æng B×nh", 167,
        "Song Th­¬ng T­íng §æng B×nh", 30 * 60
    }, {
    ...
    {
        "Song Tiªn H« Diªn Ch­íc", 167, "Song Tiªn H« Diªn Ch­íc",
        30 * 60
...
function GetTB()
    local szSay = {
        ...
        "Trang bÞ S¸t Tinh/Get_ST", "Trang bÞ ChiÕn Tr­êng/GetCT",
...
function GetCTEnd(ID1, ID2, ID3, nQH)
    ...
    if nQH == 2 then
        tinsert(szSay,
            "Trang søc 4/#Mod_GetTBCT_GetAttribute(nTBCT_ATT_NB + 64 * 3,102)")
    end
    tinsert(szSay,
        "Vò khÝ/#Mod_GetTBCT_GetAttribute(nTBCT_ATT_VK,nTBCT_ATT_A)")
...
function Get_TieuDung()
    local nRoute = GetPlayerRoute();
    local tSay = {
        "NhËn Lac/GetLac", "NhËn 10000 vµng/Get_Money",
        "NhËn §¸ Quý/Get_Gem", "NhËn vËt phÈm Shop/Get_VPSHOP",
    ...
    if 3 == nRoute then
        tinsert(tSay,
            "NhËn Thiªn PhËt Ch©u vµ Ph¸ Ma Chó/Give_ZhuzhuZhouzhou")
    end
    if 6 == nRoute then
...
function Get_Orther()
    local szSay = {
        g_szTitle .. "Lùa chän", "Th¨ng ChuyÓn Sinh 7 cÊp 99/UpSuper",
        "Thao t¸c MËt TÞch/Get_Book", "Thao t¸c Kinh M¹ch/GetJingMai",
        "Thao t¸c Thó C­ng/Pet_OP", "Thao t¸c Bang Héi/TongOperation",
        "Thao t¸c Vò KhÝ/PS_VK", "LuyÖn max skill trÊn ph¸i/maxtranphai",
...
function PS_VK()
    local szSay = {
        g_szTitle .. "H·y lùa chän hiÖu øng vò khÝ!", "ThiÕt Cèt/PS_1",
        "B¸ch ChiÕn/PS_2", "Ch­íc NhËt/PS_3", "TuÊn DËt/PS_4",
        "§µo Lý/PS_5", "Danh Tóc/PS_6", "L¹c Hµ/PS_7", "L¨ng Tiªu/PS_8",
        "PhÇn V©n/PS_9", "§o¹n Giao/PS_10", "C¸i ThÕ/PS_11",
        "Ph¸ Qu©n/PS_12", "Hµo m«n/PS_13", "\nRa khái/nothing"
    };
    SelectSay(szSay);
end
...
function enter_mp(nRoute)
    ...
    Msg2Player(
        "B¹n nhËn ®­îc 9,999,999 Kim PhiÕu hç trî phiªu l­u !!! Loli iz da bezt")
