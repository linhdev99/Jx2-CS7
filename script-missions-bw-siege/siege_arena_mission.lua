...
function talk_to_npc()
    ...
    if nType ~= 0 then
        ...
        --		else
        local tb_dialog = {"Rêi khái/no_say"};
        ...
        Say("L«i ®µi C«ng thµnh chiÕn lµ trËn ®Êu giµnh t­ c¸ch C«ng thµnh chiÕn gi÷a c¸c bang héi.",
            getn(tb_dialog), tb_dialog);
...
function kan_shuoming()
    Say("Bªn d­íi lµ thuyÕt minh L«i ®µi C«ng thµnh chiÕn.", 5, "§iÒu kiÖn b¸o danh/baoming_shuoming",
        "Vµo l«i ®µi/jinru_shuoming", "Thêi h¹n L«i ®µi/shixian_shuoming",
        "Quy t¾c ph©n th¾ng b¹i L«i ®µi/shengfu_shuoming", "Rêi khái/no_say")
end

function baoming_shuoming()
    Say(
        "ChØ cã bang chñ nhËn chøc 7 ngµy trë lªn, tæ ®éi ®ñ 8 ng­êi nhËp bang trong 7 ngµy trë lªn vµ ®éi viªn ph¶i ®¹t cÊp 80 trë lªn míi ®­îc b¸o danh, ®ång thêi ph¶i cã th­ tiÕn cö lÊy ë Thµnh §« Chu ThiÖn Nh©n, tham kh¶o thêi gian b¸o danh trong Quy tr×nh L«i §µi.",
        2, "trë l¹i/kan_shuoming", "Rêi khái/no_say")
end

function jinru_shuoming()
    Say(
        "Tham gia trËn ®Êu ph¶i do Bang chñ, phã bang chñ hoÆc Tr­ëng l·o dÉn d¾t ®éi, tÊt c¶ thµnh viªn bæn bang ph¶i tõ cÊp 80 trë lªn, vµ ph¶i nhËp bang tõ 7 ngµy trë lªn? (kh«ng h¹n chÕ mét nhãm ®ñ 8 ng­êi vµo L«i ®µi)",
        2, "trë l¹i/kan_shuoming", "Rêi khái/no_say")
end

function shixian_shuoming()
    Say(
        "Thêi h¹n L«i ®µi ®¬n trËn lµ 7 phót, sau ®ã cã 3 phót nghØ. Trong 3 phót, ®éi th¾ng trËn ph¶i chuÈn bÞ tiÕp tôc vµo khu vùc, ®éi míi khiªu chiÕn trong 3 phót ph¶i vµo khu vùc, nÕu kh«ng xem nh­ tõ bá quyÒn lîi. TÊt nhiªn sÏ cã hÖ thèng th«ng b¸o thêi gian vµo khu vùc.",
        2, "trë l¹i/kan_shuoming", "Rêi khái/no_say")
end

function shengfu_shuoming()
    Say(
        "Quy t¾c duy nhÊt cña L«i ®µi lµ khi toµn bé thµnh viªn tiÒu ®éi bÞ ®¸nh b¹i, L«i ®µi buéc ph¶i kÕt thóc. NÕu trong thêi h¹n kh«ng ph©n th¾ng b¹i, thµnh viªn ®éi nµo cßn l¹i nhiÒu h¬n sÏ th¾ng trËn, sè thµnh viªn cßn l¹i cña hai ®éi t­¬ng ®­¬ng th× ®éi cã tæng ®iÓm kinh nghiÖm cao h¬n sÏ th¾ng, nÕu ®iÓn kinh nghiÖm còng t­¬ng ®­¬ng th× sÏ chän ngÉu nhiªn quyÕt ®Þnh ®éi nµo sÏ nhËn ®uîc t­ c¸ch C«ng thµnh.",
        2, "trë l¹i/kan_shuoming", "Rêi khái/no_say")
end

function canjia_leitai()
    -- local nState = KMissionRecordRoom:GetState(CITY_SIGNUP_MAP_ID)
    -- local nState = tonumber(date("%M")) - math.floor(tonumber(date("%M"))/5)*5
    -- local nState = math.fmod(tonumber(date("%M")), 5)
    local nSecond = tonumber(date("%S"))
    local nState = 0
    if nSecond < 5 then
        nState = 0
    elseif nSecond < 20 then
        nState = 1
    elseif nSecond < 25 then
        nState = 2
    elseif nSecond < 50 then
        nState = 3
    ...
    if nState == NO_START then
        Say(
            "Thêi gian L«i ®µi b¸o danh: 19:30 tèi thø s¸u b¾t ®Çu b¸o danh\n19:30 tranh t­ c¸ch Thµnh §« C«ng thµnh L«i ®µi.",
            0)
    ...
    elseif nState == SIGNUP_TIME then
        ...
        Say(
            "B©y giê lµ thêi gian b¸o danh C«ng thµnh L«i ®µi, b¹n muèn tham gia?\rThêi gian b¸o danh cßn <color=yellow>" ..
                nMin .. " Phót " .. nSec .. " gi©y<color>", 3, "B¸o danh/signup_yes",
            "KiÓm tra c¸c ®éi b¸o danh/view_sign_team", "Ta chØ ghÐ ngang qua th«i./no_say")
...
function ViewFightState()
    ...
    szMsg = replace(szMsg, "|", "-")
    Say(
        "G©y giê lµ thêi gian chuÈn bÞ cho c¸c ®éi tham gia C«ng thµnh L«i ®µi, mêi hai ®éi tham gia thi ®Êu tiÕn vµo L«i ®µi, ®éi kh«ng tham gia xin nhÊp vµo quan s¸t trËn ®Êu ®Ó vµo L«i ®µi. Thêi gian chuÈn bÞ cßn <color=green>" ..
            nMin .. " Phót " .. nSec ..
            "gi©y<color>\n<color=red>kh«ng cã ®éi bá trËn, qu¸ thêi gian chuÈn bÞ vÉn ch­a vµo thi ®Êu xem nh­ bá cuéc<color>\n" ..
            szMsg, 2, "\nTiÕn vµo L«i ®µi/EnterFight", --		"ÎÒÒª¹ÛÕ½/EnterView", 
...
function EnterFight()
    ...
    Say("§èi thñ cña ng­¬i lµ " .. szFightTeam .. ", Muèn vµo trËn kh«ng??", 2, "Vµo/enter_yes",
        "Ta ®ang chuÈn bÞ/no_say")
end
...
function CanSignup()
    ...
    if gf_GetTeamSize() < 0 then
        Talk(1, "",
            "Ph¶i tæ ®éi vµ sè thµnh viªn lµ 8 ng­êi míi ®­îc tham gia L«i ®µi C«ng thµnh chiÕn.")
        return 0
    end
    ...
    if IsTongMaster() == 0 then
        Talk(1, "",
            "ChØ cã bang chñ lµm ®éi tr­ëng dÉn ®éi míi ®­îc b¸o danh tham gia L«i ®µi chiÕn.")
        return 0
    end
    ...
    for index, value in tFightCityName do
        ...
        if szTongName == szAttack then
            Talk(1, "",
                "B¹n ®· lµ ng­êi cña " .. value .. " (phe C«ng), kh«ng ®­îc b¸o danh tham gia thi ®Êu.")
            return 0
        end
    ...
    local nNow = GetTime()
    if nNow - nLastAssignTime < 0.604800 then
...
function CanEnter()
    ...
    if nTeamSize + nJoinCount > 8 then
        Talk(1, "", "Quý bang ®· cã <color=yellow>" .. nJoinCount ..
            " <color> ®éi viªn trong L«i ®µi råi, sè thµnh viªn ®éi b¹n hiÖn lµ <color=yellow>" ..
            nTeamSize ..
            "ng­êi<color>, L«i ®µi C«ng thµnh chiÕn cho phÐp nhiÒu nhÊt <color=yellow>8 ng­êi<color> vµo.")
        return 0
    end
    ...
    if not (IsTongMember() == 1 or IsTongMember() == 2 or IsTongMember() == 3) then
        Talk(1, "",
            "ChØ cã bang chñ, phã bang chñ hoÆc tr­ëng l·o d¾t ®éi míi ®­îc tham gia L«i ®µi chiÕn.")
        return 0
    end
...
function get_type(szKey, nKey1, nKey2, nCount)
    ...
    if nType == 1 then -- ¹¥³ÇÕ½ÀÞÌ¨
        ...
        if nHour == 19 and nMin < 40 then
            tinsert(tb_dialog, 1, "B¸o danh/baobing_jingtoutiaozhanling");
            Say(
                "<color=green> Chñ l«i ®µi<color>: HiÖn t¹i lµ thêi gian ®Êu gi¸ khiªu chiÕn lÖnh, ng­¬i muèn tham gia?",
                getn(tb_dialog), tb_dialog);
        elseif nHour == 19 or (nHour == 20 and nMin < 10) then
            tinsert(tb_dialog, 1, "Ta muèn ®Êu gi¸ khiªu chiÕn lÖnh/jingtou_tiaozhanling");
            tinsert(tb_dialog, 2, "Muèn xem t×nh h×nh ®Êu gi¸ khiªu chiÕn lÖnh/view_tiaozhanling");
            Say(
                "<color=green> Chñ l«i ®µi<color>: HiÖn t¹i lµ thêi gian ®Êu gi¸, h·y gÊp rót ®Êu gi¸.!",
                getn(tb_dialog), tb_dialog);
        else
            local szGong, szShou = GetCityWarAD(CITY_ID_CD);
            if szGong == "" then -- Èç¹ûÃ»ÓÐ¹¥³Ç·½
                if SubWorldID2Idx(CITY_ID_CD) >= 0 then -- Ö»ÔÚÄ¿±ê³ÇÊÐµØÍ¼Ëù´¦µÄ·þÎñÆ÷ÉÏ·¢ÏûÏ¢
                    if szShou ~= "" then
                        Say("<color=green> Chñ l«i ®µi<color>: Do v×" .. tSignUpCityName[CITY_ID_CD] ..
                                " C«ng thµnh chiÕn kh«ng cã phe c«ng" .. tSignUpCityName[CITY_ID_CD] ..
                                "Thµnh tiÕp tôc do " .. szShou .. " bang héi chiÕm lÜnh", 0);
                    else
                        Say("<color=green> Chñ l«i ®µi<color>: Do v×" .. tSignUpCityName[CITY_ID_CD] ..
                                " C«ng thµnh chiÕn kh«ng cã phe c«ng vµ phe thñ, v× vËy " ..
                                tSignUpCityName[CITY_ID_CD] .. " C«ng thµnh chiÕn t¹m dõng 1 trËn", 0);
                    end
                end
            else
                Say(
                    "<color=green> Chñ l«i ®µi<color>: Qua mét kú ®Êu gi¸ n¶y löa, cuèi cïng còng chän mét bang héi cã t­ c¸ch c«ng thµnh cña tuÇn nµy. <enter>Ta trÞnh träng tuyªn bè, bªn c«ng thµnh cña tuÇn nµy lµ: " ..
                        szGong .. "<enter>Chóc mõng bang héi ®· thµnh c«ng trong tuÇn nµy.", 0);
            end
        end
    end
    -- end
